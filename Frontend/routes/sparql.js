...
var endpoint = "http://localhost:3030/KMSWT-Movie-dataset/sparql";
var myupdateEndpoint =
  "http://localhost:3030/dataset/KMSWT-Movie-dataset/query";

var url = require("url");

// retrieves an array of all the keys or property names of the persons object
...
router.get("/explore", function (req, res, next) {
  ...
  //array com o nome de todos os filmes
    ...
    var query =
      "select ?nome  ?rating ?poster where{ \n" +
      "?s rdf:type cin:Movie .      \n" +
    ...
    client
      ...
      .then((qres) => {
        ...
        console.log("\n\n" + JSON.stringify(top));
        // The data object contains several properties, each of which corresponds to a data source that will be available
        // in the Pug template. These data sources are being passed to the template in order to render dynamic content.
        res.render("explore", {
...
function putStruturedResult_Movies(resList) {
  ...
  for (i = 0; i < resList.length; i++) {
    ...
    if (property == "type") result.type.push(value);
    ...
    //dataproperties
    if (property == "name") result.name = value;

    if (property == "poster") result.poster = value;

    if (property == "numVotes") result.numVotes = value;

    if (property == "runtime") result.runtime = value;

    if (property == "plot") result.plot = value;

    if (property == "boxOffice") result.boxOffice = value;

    if (property == "website") result.website = value;

    if (property == "premiereYear") result.premiereYear = value;

    if (property == "rating_RT") result.rating_RT = value;

    if (property == "rating_IMD") result.rating_IMD = value;

    if (property == "rating_META") result.rating_META = value;
  }
...
function putStruturedResult_Person(resList) {
  ...
  for (i = 0; i < resList.length; i++) {
    ...
    if (property == "birthYear") result.birthYear = value;

    if (property == "deathYear") result.deathYear = value;

    if (property == "sex") result.sex = value;
  }
